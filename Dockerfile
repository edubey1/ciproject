...
ENV DEMOAPP_BUNDLE_JAR $DEMOAPP_PATH/core/target/aemdemo.core-0.0.5-SNAPSHOT.jar
ENV DEMOAPP_CONTENT_PACKAGE $DEMOAPP_PATH/ui.content/target/aemdemo.ui.content-0.0.5-SNAPSHOT.zip
ENV DEMOAPP_UI_PACKAGE $DEMOAPP_PATH/ui.apps/target/aemdemo.ui.apps-0.0.5-SNAPSHOT.zip
...
RUN echo sling.run.modes=${CQ_RUNMODE} >> crx-quickstart/conf/sling.properties && \
    ...
    curl -u admin:admin --output /dev/null -F file=@"$DEMOAPP_CONTENT_PACKAGE" -F name="aemdemo.ui.content-0.0.5-SNAPSHOT" -F force=true -F install=true http://localhost:$CQ_PORT/crx/packmgr/service.jsp && \
    curl -u admin:admin --output /dev/null -F file=@"$DEMOAPP_UI_PACKAGE" -F name="aemdemo.ui.apps-0.0.5-SNAPSHOT" -F force=true -F install=true http://localhost:$CQ_PORT/crx/packmgr/service.jsp && \
    sleep 10 && \
